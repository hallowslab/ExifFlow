...
The backend links the workspace `rftps` crate with the `broker` feature, so the
...
### 2. Replication via the Broker

Replication offloads FTP/FTPS target credentials to the broker gateway, so
nothing sensitive sits on the client.

1. Make sure a broker is running — see
   [`../access-broker/README.md`](../access-broker/README.md).
2. Open **Settings → Broker**.
3. Set **Broker URL** (e.g. `http://127.0.0.1:8700`) and a **Device Name**.
   **Device Key** is auto-generated if left blank.
4. Click **REGISTER DEVICE** — the device appears on the broker dashboard.
5. Approve it at `http://127.0.0.1:8701/dashboard` (and configure the storage
   target there if you haven't).
6. Start the FTP server and upload a file. A `BROKER: ACTIVE` badge confirms the
   connection, and the file replicates to the configured storage.

Optional toggles in the same section:

* **Print broker messages** — on by default; switch off for less verbose log
...
| Invoke name | Purpose |
| ----------- | ------- |
| `start_ftp_server` | Start FTP/FTPS server with optional broker replication |
| `stop_ftp_server` | Stop the FTP server |
| `get_server_info` | Resolve the machine's LAN address for display |
| `register_broker_device` | Register the device at the broker and wait for approval |
| `run_organization` / `stop_organization` | Run/stop the EXIF organizer |
| `run_backup` | Run a deduplicated backup |

## Settings storage

Settings (including broker config) persist in WebView `localStorage` under the
